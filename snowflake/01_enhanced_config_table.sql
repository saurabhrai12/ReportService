...
-- Create a single, unified view for all pending reports
CREATE OR REPLACE VIEW REPORTING_DB.CONFIG.V_PENDING_REPORTS AS
SELECT
    CONFIG_ID,
    REPORT_NAME,
    REPORT_TYPE,
...
WHERE
    ...
    AND IS_ACTIVE = TRUE
    AND (
        TRIGGER_TYPE = 'ADHOC' OR
        (TRIGGER_TYPE = 'SCHEDULED' AND (NEXT_RUN_TIME IS NULL OR NEXT_RUN_TIME <= CURRENT_TIMESTAMP()))
    )
...
INSERT INTO REPORTING_DB.CONFIG.REPORT_CONFIG (
      ...
      'TEST_ADHOC_20250916', 'Test ADHOC Report', 'TEST', 'ADHOC', 1
  );
...
-- Update scheduled reports to future times
  UPDATE REPORTING_DB.CONFIG.REPORT_CONFIG
  SET NEXT_RUN_TIME = DATEADD(hour, 24, CURRENT_TIMESTAMP())
